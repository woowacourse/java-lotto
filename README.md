...
- [x] `LottoPrize`에서 `checkMatches`의 조건 분기를 합칠 수 있지 않을까?
    - [ ] 수정한 stream식이 너무 복잡하지 않을까? 특정 조건을 early return을 하는 방식이면 훨씬 쉽게 해결이 될 것 같다.
...
- [ ] Lotto를 만들 때 동일한 LottoNumber라면 같은 인스턴스를 사용할 수 있도록 캐싱을 해보면 어떨까요?
    - 힌트
        - 캐싱할 자료구조를 고민한다.
