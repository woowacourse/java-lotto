...
예외가 발생하면 입력을 재시도한다.
- 보너스 볼 입력이 당첨 번호와 중복된다면 예외가 발생한다.
- 구입금액 입력
  - 로또 가격으로 나누어떨어지지 않으면 예외가 발생한다.
  - 0원이라면, 예외가 발생한다.
  - 숫자가 아니라면, 예외가 발생한다.
  - 양수가 아니라면, 예외가 발생한다.
  - int 범위 내의 값이 아니라면, 예외가 발생한다.
  - 
- 당첨 번호 입력
  - `,` 이외의 문자가 들어온다면, 예외가 발생한다.
  - 당첨 번호들이 숫자가 아니라면, 예외가 발생한다.
  - int 범위 내의 값이 아니라면, 예외가 발생한다.
  - 당첨 번호들에 중복이 존재한다면, 예외가 발생한다.
  - 로또 번호 이외의 수가 포함되어있다면, 예외가 발생한다.
  - 당첨 번호들의 수가 6개가 아니라면, 예외가 발생한다.
- 보너스 볼을 입력
  - 로또 번호 이외의 수가 포함되어있다면, 예외가 발생한다.
  - 당첨 번호에 포함된 수라면, 예외가 발생한다.
  - 숫자가 아니라면, 예외가 발생한다.
  - int 범위 내의 값이 아니라면, 예외가 발생한다.
