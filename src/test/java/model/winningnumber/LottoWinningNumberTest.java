...
public class LottoWinningNumberTest {
	...
	void validateWinningNumberOutOfRange() {
		assertThatThrownBy(() -> new LottoWinningNumber(Arrays.asList(1, 2, 3, 4, 5, 0).stream()
	...
	void validateInputLottoWinningNumberSize() {
		assertThatThrownBy(() -> new LottoWinningNumber(Arrays.asList(1, 2, 3, 4, 5).stream()
			.map(number -> LottoNumber.valueOf(number))
			.collect(Collectors.toList())))
			.isInstanceOf(IllegalArgumentException.class)
	...
	void validateWinningNumberReduplication() {
		assertThatThrownBy(() -> new LottoWinningNumber(Arrays.asList(1, 2, 3, 4, 5, 1).stream()
			.map(number -> LottoNumber.valueOf(number))
			.collect(Collectors.toList())))
			.isInstanceOf(IllegalArgumentException.class)
	...
	void validateReduplicationWithBonusBall() {
		LottoWinningNumber lottoWinningNumber = new LottoWinningNumber(Arrays.asList(1, 2, 3, 4, 5, 6).stream()
			.map(number -> LottoNumber.valueOf(number))
			.collect(Collectors.toList()));
		assertThatThrownBy(() -> lottoWinningNumber.validateReduplicationWithBonusBall(LottoNumber.valueOf(1)))
			.isInstanceOf(IllegalArgumentException.class)
