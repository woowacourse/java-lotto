...
public class ChoiceNumberTest {
    ...
    void Generate_6_Number() {
        ...
        assertThat(choiceNumber.getChoiceNumbers().size()).isEqualTo(6);
    }
    ...
    void Is_Number_Format() {
        assertThatThrownBy(() -> new ChoiceNumber("1,2,3,a,b,c")).isInstanceOf(IllegalArgumentException.class);
    }
    ...
    void Get_6_Input() {
        ...
        assertThat((long) choiceNumber.getChoiceNumbers().size()).isEqualTo(6);
    }
    ...
    void No_6_Input_Numbers(String input) {
        assertThatThrownBy(() -> new ChoiceNumber(input)).isInstanceOf(IllegalArgumentException.class);
    }

    @Test
    @DisplayName("입력받은 당첨번호가 모두 범위내에 있는지")
    void All_Numbers_In_Range() {
        assertThatThrownBy(() -> new ChoiceNumber("0,2,3,4,5,6")).isInstanceOf(IllegalArgumentException.class);
    }

    @Test
    @DisplayName("입력받은 당첨번호가 중복되는 값이 있는지")
    void Duplicate_Number_Exist() {
        assertThatThrownBy(() -> new ChoiceNumber("1,1,3,4,5,6")).isInstanceOf(IllegalArgumentException.class);
    }
