...
class LottoFactoryTest {
	@Test
	void 생성_테스트() {
		assertThat(LottoFactory.createAuto());
	}

	@Test
	@DisplayName("로또 장수 만큼 로또를 반환하는지")
	void LottosTest() {
		Lottos lottos = LottoFactory.createAutosOf(3);
	...
	void 로또_수동_생성_테스트(String input, int expected) {
		Lotto lotto = LottoFactory.createManual(input);
	...
	void manualWithAutoLotto() {
		...
		assertThat(LottoFactory.createManualsAndAutos(manualInput, 7)).hasSize(9);
	}
	...
	void lottosHasManualLotto() {
		...
		Lotto manualLotto = new Lotto(Arrays.asList(
			...
			Number.of("5"),
			Number.of("6")));
		Iterator<Lotto> iter = LottoFactory.createManualsAndAutos(manualInput, 7).iterator();
