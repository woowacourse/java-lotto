...
class WinningLottoParserTest {
    ...
    void parse_가징작은_가능한숫자보다_적은숫자포함() {
        ...
        assertThrows(NumberOutOfRangeException.class, () -> WinningLottoParser.parse(input));
    }
    ...
    void parse_로또에_필요한_숫자개수_부족() {
        ...
        assertThrows(LottoExceedSizeException.class, () -> WinningLottoParser.parse(input));
    }
    ...
    void parse_중복있는_숫자() {
        ...
        assertThrows(LottoDuplicationException.class, () -> WinningLottoParser.parse(input));
    }

    @Test
    void parse_정렬된_올바른입력() {
        String input = "1, 2, 3, 4, 5, 6";
        WinningLotto expected = WinningLotto.of(
                LottoGenerator.from(Arrays.asList(1, 2, 3, 4, 5, 6)), Number.from(Number.NUMBER_TO));

        assertThat(WinningLottoParser.parse(input)).isEqualTo(expected);
    }
    ...
    void parse_정렬안된_올바른입력() {
        ...
        WinningLotto expected = WinningLotto.of(
                LottoGenerator.from(Arrays.asList(40, 41, 42, 43, 44, 45)), Number.from(Number.NUMBER_TO));
