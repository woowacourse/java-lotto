...
public class LottoGameTest {
	...
	class CheckNumbersInLottoTest {
		@Test
		@DisplayName("6개이면 올바른 로또이다.")
		void valid_lotto_number(){
	...
	class CheckNumberRangeTest {
		...
		void valid_lotto_number_range() {
			assertThatNoException().isThrownBy(() -> new Lotto(Arrays.asList(1, 2, 4, 5, 6, 45)));
		}

		@Test
		@DisplayName("1~45 사이가 아니면 실패")
		void invalid_lotto_number_range() {
			assertThatThrownBy(() -> {
				new Lotto(Arrays.asList(-1, 0, 46, 3, 4, 5));
			}).isInstanceOf(IllegalArgumentException.class)
