...
public class Lotto {
    ...
    public LottoMatchResult getMatchResult(final List<Integer> matchNumbers, final int bonusNumber) {
    ...
    private void validateMatchNumbersNotDuplicated(final List<Integer> matchNumbers) {
        if (matchNumbers.size() != new HashSet<>(matchNumbers).size()) {
            throw new IllegalArgumentException("로또 당첨 번호는 중복되면 안됩니다.");
        }
    }

    private void validateMatchNumbersSize(final List<Integer> matchNumbers) {
        if (matchNumbers.size() != NUMBER_COUNT) {
            throw new IllegalArgumentException("로또 당첨 번호는 6개여야 합니다.");
        }
    }

    private void validateBonusNumberNotDuplicated(final List<Integer> matchNumbers, final int bonusNumber) {
        if (matchNumbers.contains(bonusNumber)) {
            throw new IllegalArgumentException("보너스 번호는 중복되면 안됩니다.");
        }
    }

    private int calculateMatchNumberCount(final List<Integer> matchNumbers, int matchCount) {
