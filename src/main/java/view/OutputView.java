...
public class OutputView {
    ...
    public static void showLottoTicket(List<LottoTicketDto> lottoTickets, int selfPurchaseCount) {
    ...
    private static String toLottoTicketPrintForm(LottoTicketDto lottoTicket) {
        return RIGHT_LIST_COVER + joiningLottoNumbers(lottoTicket) + LEFT_LIST_COVER;
    }

    private static String joiningLottoNumbers(LottoTicketDto lottoTicket) {
        return lottoTicket.getLottoNumbers().stream()
                .map(Object::toString)
                .collect(Collectors.joining(LIST_JOINING_DELIMITER));
    }

    public static void showLottoResult(List<WinningResultDto> winningResults, double rateOfReturn) {
        System.out.println(WINNING_STATISTICS_MESSAGE);
        System.out.println(DIVISION_LINE);
        winningResults.forEach(OutputView::printWinningResult);
        System.out.printf(TOTAL_RETURN_MESSAGE, rateOfReturn);
    }

    private static void printWinningResult(WinningResultDto winningResult) {
    ...
    private static String makeWinningMessage(boolean matchBonus) {
