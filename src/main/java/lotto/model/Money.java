...
public class Money {
    ...
    private double getRevenue(LottoResult lottoResult) {
        return Arrays.stream(RankType.values())
            .mapToDouble(rankType -> rankType.getPrize() * lottoResult.getResultCount(rankType))
            .sum();
    }

    public int getYield(LottoResult lottoResult) {
        return (int) (getRevenue(lottoResult) / money * PERCENT);
    }
}
