...
import static com.woowacourse.lotto.view.OutputView.PRINT_RESULT_OF_LOTTO;
...
public class WebUILottoApplication {
	...
	public static void main(String[] args) throws SQLException {
		...
		post("inputWinningLotto", (request, response) -> {
			try {
				...
				List<String> winningLottoNumbers = Arrays.asList(request.queryParams("winningLotto").split(","));
		...
		post("/generateLottos", (request, response) -> {
			try {
				...
				if (countOfManualLotto > 0) {
					list = Arrays.asList(request.queryParams("manualLottos").split("\\r\\n"));
				}
		...
		post("/matchLotto", (request, response) -> {
			try {
				...
				StringBuilder matchedResult = new StringBuilder();
				for (LottoRank lottoRank : LottoRank.values()) {
					matchedResult.append(String.format(PRINT_RESULT_OF_LOTTO + NEW_LINE, lottoRank.getCount(), lottoRank.getPrice(), lottoResult.valueOf(lottoRank)));
				}
				LottoMoney lottoMoney = request.session().attribute("lottoMoney");
				long earningRate = lottoMoney.calculateEarningsRate(lottoResult.sum());
				addLottoResult(request.session().attribute("round"), lottoMoney, lottoResult);
				Map<String, Object> model = new HashMap<>();
				model.put("matchedResult", matchedResult.toString());
		...
		post("/searchLottoResult", ((request, response) -> {
			try {
				...
				model.put("earningRate", String.format(PRINT_EARNINGS_RATE + NEW_LINE, findLottoResultSumAndEarningRateById(selectedRound).get("earningRate")));
				model.put("sum", String.format(PRINT_SUM + NEW_LINE, findLottoResultSumAndEarningRateById(selectedRound).get("sum")));
				return render(model, "/searchLottoResult.html");
			} catch (Exception e) {
				Map<String, Object> model = new HashMap<>();
				model.put("errorMessage", e.getMessage());
	...
	private static String findLottoResultRankById(int selectedRound) throws SQLException {
		Map<LottoRank, Integer> ranks = lottoResultDAO.findLottoResultRankById(selectedRound);
